...
void Custom_GPIO_Init (void)
{
    ...
    // Initialize the GPIO pin (output high level, pull up)
    Ql_GPIO_Init(gpioPin, PINDIRECTION_OUT, gpioLvl, PINPULLSEL_PULLUP);  
    APP_DEBUG("<-- Custom GPIO_Init LED Fashined!!! -->\r\n");   
...
void Custom_GPIO_Init_IIC(void)
{
   // Specify a GPIO For IIC
    ...
    APP_DEBUG("<-- Custom GPIO_Init IIC_SCL Fashined!!! -->\r\n");

  // Specify a GPIO For SDA
    Ql_GPIO_Init(PINNAME_GPIO5,PINDIRECTION_OUT, PINLEVEL_HIGH, PINPULLSEL_PULLUP);  
    APP_DEBUG("<-- Custom GPIO_Init IIC_SDA Fashined!!! -->\r\n");
...
//    Enum_PinName  gpioPin = PINNAME_GPIO3;
 ...
 //   APP_DEBUG("<-- Set the GPIO level to high after 500ms -->\r\n");
    Ql_Sleep(500);
    Ql_GPIO_SetLevel(PINNAME_GPIO3, PINLEVEL_HIGH);
//    APP_DEBUG("<-- Get the GPIO level value: %d -->\r\n", Ql_GPIO_GetLevel(gpioPin));
//    Ql_Delay_us(10);
