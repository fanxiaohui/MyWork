 ...
 * 
 ****************************************************************************/
#ifndef __EXAMPLE_ONENET__
#define __EXAMPLE_ONENET__
#endif
...
#define ONENET_TEMP_BUFFER_LENGTH 1024
...
Onenet_Param_t onnet_param_t= {0,0,0,0,0,0,0,0,0}; //deal with business  parameters.
			...
			switch (msg.param1)
            {
				...
				case URC_ONENET_OBSERVE:// Indication for  Notify the TE an Observe Request.
				{
					...
					if( onenet_obj_param_t.ref  == onenet_urc_param_ptr->ref &&observe_flag ==FALSE)
					{
						...
						onnet_param_t.ref = onenet_urc_param_ptr->ref;//Instance ID of OneNET communication suite.
						onnet_param_t.observe_msg_id = onenet_urc_param_ptr->msgid;//The message id of packet.
						onnet_param_t.obj_id  = onenet_urc_param_ptr->objid;//The object identifier,eg 3203
						onnet_param_t.ins_id = onenet_urc_param_ptr->insid; //Instance identifier,eg 0
    			...
    			break;
		        case URC_ONENET_WRITE:// Indication for Notify the TE to Respond the Write Request 
				{
					onenet_urc_param_ptr = msg.param2;
			     	if( onenet_obj_param_t.ref  == onenet_urc_param_ptr->ref)
					{
    ...
    if (ONENET_TIMER_ID == timerId)
    {
        ...
        {        
            ...
            case STATE_ONENET_NOTIFY:
            {
			    ...
			    onenet_notify_param_t.objid =  onnet_param_t.obj_id; //eg  3203
			    onenet_notify_param_t.insid =  onnet_param_t.ins_id; //eg  0
			    onenet_notify_param_t.resid = onnet_param_t.res_id;  //eg 5750
			    onenet_notify_param_t.value_type = ONENET_VALUE_TYPE_OPAQUE; //send data mode.
			    onenet_notify_param_t.len = Ql_strlen("30313233343536\0");//data
				...
				Ql_strncpy(onenet_notify_param_t.value,"30313233343536\0",Ql_strlen("30313233343536\0")); 
