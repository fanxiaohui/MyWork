 ...
 *   Compile & Run:
 *
 *     Set "C_PREDEF=-D __EXAMPLE_SOCKET_TEST__" in gcc_makefile file. And compile the 
		...
		{     
			switch (msg.param1)
            {
    		    ...
    		    case URC_SOCKET_RECV_DATA:
    				...
    				{
						APP_DEBUG("+QIURC: \"%s\",%d,%d\r\n", "recv",socket_recv_param_ptr->connectID,socket_recv_param_ptr->recv_length);
    				}
    				else if(socket_recv_param_ptr->access_mode == SOCKET_ACCESS_MODE_DIRECT)
    				{ 
    					if ( recv_data_format == 0 ) //text
    					{
							APP_DEBUG("+QIURC: \"%s\",%d,%d,", "recv",socket_recv_param_ptr->connectID,\
							 	socket_recv_param_ptr->recv_length);
							Ql_UART_Write(UART_PORT0, socket_recv_param_ptr->recv_buffer, socket_recv_param_ptr->recv_length);
							APP_DEBUG("\r\n");
    					}
						else if ( recv_data_format == 1 ) //hex
						{
							APP_DEBUG("+QIURC: \"%s\",%d,%d,%s", "recv",socket_recv_param_ptr->connectID,\
...
static void proc_handle(u8 *pData,s32 len)
{
    ...
    if (p)
    {
		...
		socket_param_t.address = m_SrvADDR;
	...
	if (p)
	{
		...
		{
			m_socketid = ret; 
			APP_DEBUG("+QIOPEN: %d,0\r\n",m_socketid);
		}
		else
		{
			APP_DEBUG("+QIOPEN: %d,%d\r\n",socket_param_t.connectID,ret);
		}  	
		return;
	}
	...
	if (p)
	{
   	...
   	//command: SOCKET_QCFG=<send_data_format>,<recv_data_format>
    p = Ql_strstr(pData,"SOCKET_QCFG=");
	if (p)
	{
		Ql_memset(temp_buffer, 0, TEMP_BUFFER_LENGTH);		
		if (Analyse_Command(pData, 1, '>', temp_buffer))
		{
			APP_DEBUG("SOCKET config recv mode  Error.\r\n");
			return;
		}
		send_data_format = Ql_atoi(temp_buffer);
   	    Ql_memset(temp_buffer, 0, TEMP_BUFFER_LENGTH);
		if (Analyse_Command(pData, 2, '>', temp_buffer))
		{
			APP_DEBUG("SOCKET config recv mode  Error.\r\n");
		...
		recv_data_format = Ql_atoi(temp_buffer);
		ret = RIL_SOC_QICFG_FORMAT(send_data_format, recv_data_format);
		if (ret == 0)
		{
			APP_DEBUG("OK\r\n");
		}else
